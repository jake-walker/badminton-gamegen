...
export async function resolvePlayerIds(
  groupId: string,
  players: (typeof schema.player.$inferSelect)[],
  names: (string | null)[],
): Promise<(string | null)[]> {
  return Promise.all(
...
async function updatePlayerElo(
  ...
  await db
    ...
    .where(
      and(
        eq(schema.matchPlayer.playerId, playerId),
        eq(schema.matchPlayer.matchId, matchId),
      ),
    );
}

export async function updateEloRankings(
  matchId: string,
  winnerPlayerIds: (string | null)[],
  loserPlayerIds: (string | null)[],
) {
  const winners = await Promise.all(
    winnerPlayerIds.map(async (playerId) => ({
      playerId,
      currentRank: await getPlayerEloRank(playerId),
    })),
  );
  const losers = await Promise.all(
    loserPlayerIds.map(async (playerId) => ({
      playerId,
      currentRank: await getPlayerEloRank(playerId),
    })),
  );
  ...
  const winnerRating =
    winners
      .map((x) => x.currentRank)
      .reduce((partialSum, a) => partialSum + a, 0) / winnerPlayerIds.length;
  const loserRating =
    losers
      .map((x) => x.currentRank)
      .reduce((partialSum, a) => partialSum + a, 0) / loserPlayerIds.length;
...
export async function createMatch(
  info: NewMatchInfo,
): Promise<typeof schema.match.$inferSelect> {
  const match = (
    await db
      .insert(schema.match)
      .values({
  ...
  const teamAMatchPlayers = await db.insert(schema.matchPlayer).values(
    info.teamAPlayerIds.map((playerId) => ({
      matchId: match.id,
      playerId,
      side: "teamA" as const,
    })),
  ...
  const teamBMatchPlayers = await db.insert(schema.matchPlayer).values(
    info.teamBPlayerIds.map((playerId) => ({
      matchId: match.id,
      playerId,
      side: "teamB" as const,
    })),
