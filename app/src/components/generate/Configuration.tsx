...
export default function GeneratorConfiguration() {
  ...
  return (
    <>
      {requiredPlayers > 0 && <Alert severity="warning" sx={{ mb: 2 }}>
        You need to add {requiredPlayers} more players to fill a full set of matches.
      </Alert>}
      ...
      <FormControl sx={{ mb: 2 }} fullWidth>
        ...
        <FormHelperText>This is the number of people on one side of the court. For example, use 1 for singles, 2 for doubles.</FormHelperText>
      </FormControl>

      <FormControl sx={{ mb: 2 }} fullWidth>
        <InputLabel id="add-player-input-label">New Player</InputLabel>
        <OutlinedInput label="New Player" value={newPlayer} onChange={(e) => setNewPlayer(e.target.value as string)} onKeyDown={handleNewPlayerKeyDown} endAdornment={
          <InputAdornment position="end">
            <IconButton aria-label="add player" edge="end" onClick={addPlayer}>
              <AddIcon />
            </IconButton>
          </InputAdornment>
        } />
