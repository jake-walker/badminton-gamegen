...
export function formatMatch(match: Match): string {
  return [match.teamA, match.teamB].map((team) => team.map((p) => p.name).join(" and ")).join(" vs. ");
}
...
export function playerGameCounts(session: Session) {
  return countOccurances(session.matches.flatMap((m) => [...m.teamA, ...m.teamB]).map((p) => p.id))
}

export function pairGameCounts(session: Session) {
  return countOccurances(session.matches.flatMap((m) => [m.teamA.map((p) => p.id), m.teamB.map((p) => p.id)]))
}

export function getNextCourt(session: Session, totalCourts: number): number | undefined {
...
export function getExcludedPlayerIds(session: Session, nextCourt: number): Array<string> {
  if (nextCourt === 0) return [];

  const otherCourtGames = session.matches.slice(-nextCourt);
  return otherCourtGames.flatMap((m) => [...m.teamA, ...m.teamB].map((p) => p.id));
}
...
export function nextGame(session: Session, courts: number): Match | null {
  ...
  const gameCandidates = combinations(combinations(players)).filter(([teamA, teamB]) => {
    const unique = new Set();

    for (const item of [...teamA, ...teamB]) {
    ...
    const aGameCount = a.flat().map((t) => playerCounts.get(t) || 0).reduce((partialSum, i) => partialSum + i, 0);
    const bGameCount = b.flat().map((t) => playerCounts.get(t) || 0).reduce((partialSum, i) => partialSum + i, 0);

    const aTeamCount = a.map((t) => pairCounts.get(JSON.stringify(t)) || 0).reduce((partialSum, i) => partialSum + i, 0);
    const bTeamCount = b.map((t) => pairCounts.get(JSON.stringify(t)) || 0).reduce((partialSum, i) => partialSum + i, 0);
