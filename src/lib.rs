...
impl Session {
    ...
    pub fn next_game(&self) -> Option<Game> {
        ...
        let mut shuffled_player_ids = self.player_names.iter().enumerate().map(|(index, _)| index).collect_vec();
        shuffled_player_ids.shuffle(&mut thread_rng());
        
        let mut game_candidates = shuffled_player_ids.into_iter().combinations(2).combinations(2).filter(|teams| {
